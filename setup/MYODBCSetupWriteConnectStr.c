...
*/  
BOOL MYODBCSetupWriteConnectStr( MYODBC_DATASOURCE *pDataSource, char *pszStr, SQLSMALLINT nMaxLen )
{
    int nIndex = 0;
    ...
    *pszStr = '\0';
    ...
    if ( pDataSource->pszDATABASE )
    {
        if ( !MYODBCSetupInsertStr( pszStr, "DATABASE=", nMaxLen, &nIndex ) )
            return FALSE;
        if ( !MYODBCSetupInsertStr( pszStr, pDataSource->pszDATABASE, nMaxLen, &nIndex ) )
            return FALSE;
    }
    ...
    if ( pDataSource->pszDRIVER )
    {
        ...
        if ( !MYODBCSetupInsertStr( pszStr, "DRIVER=", nMaxLen, &nIndex ) )
            return FALSE;
        if ( !MYODBCSetupInsertStr( pszStr, pDataSource->pszDRIVER, nMaxLen, &nIndex ) )
            return FALSE;
    }
    ...
    if ( pDataSource->pszOPTION )
    {
        ...
        if ( !MYODBCSetupInsertStr( pszStr, "OPTION=", nMaxLen, &nIndex ) )
            return FALSE;
        if ( !MYODBCSetupInsertStr( pszStr, pDataSource->pszOPTION, nMaxLen, &nIndex ) )
            return FALSE;
    }
    ...
    if ( pDataSource->pszPASSWORD )
    {
        ...
        if ( !MYODBCSetupInsertStr( pszStr, "PWD=", nMaxLen, &nIndex ) )
            return FALSE;
        if ( !MYODBCSetupInsertStr( pszStr, pDataSource->pszPASSWORD, nMaxLen, &nIndex ) )
            return FALSE;
    }
    ...
    if ( pDataSource->pszSERVER )
    {
        ...
        if ( !MYODBCSetupInsertStr( pszStr, "SERVER=", nMaxLen, &nIndex ) )
            return FALSE;
        if ( !MYODBCSetupInsertStr( pszStr, pDataSource->pszSERVER, nMaxLen, &nIndex ) )
            return FALSE;
    }
    ...
    if ( pDataSource->pszSOCKET )
    {
        ...
        if ( !MYODBCSetupInsertStr( pszStr, "SOCKET=", nMaxLen, &nIndex ) )
            return FALSE;
        if ( !MYODBCSetupInsertStr( pszStr, pDataSource->pszSOCKET, nMaxLen, &nIndex ) )
            return FALSE;
    }
